 ...
 *
 * Shows students in a classroom/learning setting.
 * Images crossfade every 5 seconds with a smooth CSS transition.
 */
...
const INTERVAL_MS = 5000; // comfortable 5-second auto-transition
  ...
  return (
    ...
    >
      ...
      <div className="relative rounded-2xl overflow-hidden border border-line shadow-elev-3">
        ...
        <div
          aria-hidden
          className="absolute inset-0 bg-gradient-to-t from-[#0B0F1A]/80 via-[#0B0F1A]/15 to-[#0B0F1A]/20 z-[3]"
        />

        {/* Live badge */}
        <div className="absolute top-3.5 left-3.5 z-[4] inline-flex items-center gap-2 px-2.5 py-1.5
                        rounded-lg bg-[#0B0F1A]/80 border border-white/12 backdrop-blur-sm">
        ...
        <div className="absolute bottom-3.5 inset-x-3.5 z-[4] grid grid-cols-3 gap-2">
          {[
            { Icon: Users, value: '12+', label: 'Yrs Experience' },
            { Icon: Video, value: '100%', label: 'Live Classes' },
            { Icon: ShieldCheck, value: '8+', label: 'Programs' },
          ].map(({ Icon, value, label }) => (
            <div
              key={label}
              className="rounded-lg bg-[#0B0F1A]/75 border border-white/10 backdrop-blur-sm px-2.5 py-2"
            >
